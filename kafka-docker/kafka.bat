...
ECHO
ECHO    /r                      This will refresh discovery-kafka docker image
ECHO                            (Only needed if the discovery-kafka Version has changed
ECHO
GOTO :end
...
:checkBase
FOR /F %%i IN ('docker images -q discovery-kafka') DO (
    ECHO Found discovery-kafka image
    GOTO :main
)
